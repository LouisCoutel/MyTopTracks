...
class AmMap {
  ...
  async getGeoOutline() {
    const worldOutlineLow = await import(
      "@amcharts/amcharts5-geodata/worldOutlineLow"
    );
  ...
  handleCountryClick = (ev) => {
    ...
    state.createEffect(() => {
      this.view.currentCountry.id =
        this.view.currentCountry.element.dataItem.dataContext.id;
      this.view.currentCountry.name =
        this.view.currentCountry.element.dataItem.dataContext.name;
  ...
  setCountriesData(data) {
    this.countriesSeries.mapPolygons.template.setAll({
